    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-7 align-self-center">
                <h4 class="page-title text-truncate text-dark text-start font-weight-medium mb-1"><?php echo $this->lang->line('TENANTS'); ?></h4>
                <div class="d-flex align-items-center">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb m-0 p-0">
                            ...
                            <li class="breadcrumb-item text-muted active" aria-current="page"><?php echo $this->lang->line('TENANTS'); ?></li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        ...
                        <?php if ($sess->ag_can_create == 1 || $sess->ag_can_update == 1 ) : ?>
                            <table id="zero_config" class="table table-striped table-bordered no-wrap">
                                ...
                                <tbody>
                                    <?php $i = 1;
                                    $tenants = count($searched_data)>0?$searched_data:$tenants;
                                    foreach ($tenants as $tenants) { ?>
                                        <tr>
                                            <?php if ($sess->ag_can_update == 1) : ?>
                                                <th>
                                                    <button title="<?php echo $this->lang->line('edit'); ?>" onclick="edittenants(<?php echo $tenants->id ?>);" type="button" class="btn btn-sm btn-light"> <i class="fas fa-edit text-primary"></i></button>
                                                </th>
                                            <?php endif; ?>
                                            <?php if ($sess->ag_can_delete == 1) : ?>
                                                <th>

                                                    <button title="<?php echo $this->lang->line('deletetenants'); ?>" onclick="deletetenants(<?php echo $tenants->id ?>)" type="button" class="btn btn-sm btn-light"><i class="fas fa-trash text-danger"></i></button>

                                                </th>
                                            <?php endif; ?>
                                            <td><?php echo $tenants->fullName; ?></td>
