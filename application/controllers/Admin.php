...
class Admin extends CI_Controller
{
	...
	public function rentrequest()
	{
		...
		$datagraph['stat'][] = $this->lang->line('TOTAL_RENT');
	...
	public function ownersDuesrequest()
	{
		...
		$datagraph1['stat'][] = $this->lang->line('PAID_OWNERS_DUE');
	...
	public function incomerequest()
	{
		...
		$datagraph['stat'][] = $this->lang->line('MANAGEMENT_FEE');
		$datagraph['cnt'][] = $dgraph[0]->cnt;
...
{
	$selectedYear = $this->input->get('filter'); // Get the 'filter' query parameter
// Construct the SQL query to filter data based on the selected year
$sql = "SELECT DATE_FORMAT(createdOn, '%b %Y') AS stat, COUNT(*) AS cnt
        FROM tbl_contracts
        WHERE YEAR(createdOn) = ?";

    
    // Bind the selected year to the query
    $query = $this->db->query($sql, array($selectedYear));
    $filter = isset($_GET['filter']) ? strval($_GET['filter']) : null;
    ...
    // Modify your model function to use the $filter variable in the SQL query
    $dgraph = $this->dashboardModel->contract_stats($filter,$year);
    ...
    $datagraph['cnt'] = $cnt;
    $datagraph['bgcolor'] = "#789de7";
    $datagraph['title'] = $filter ? $this->lang->line('CONTRACTS_REG')." ". $filter : $this->lang->line('CONTRACTS_REG');
